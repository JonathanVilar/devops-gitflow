...
# Puede leer config
if sudo -n -u "$USUARIO" cat "$APP_DIR/config/app.conf" &>/dev/null; then
...
if sudo -n -u "$USUARIO" bash -c "echo test >> $APP_DIR/config/app.conf" 2>/dev/null; then
...
# Puede escribir logs
if sudo -n -u "$USUARIO" bash -c "echo auditoria >> $APP_DIR/logs/audit.log" 2>/dev/null; then
    ok "Puede ESCRIBIR logs"
else
    fail "No puede escribir logs"
fi

#con la corrección ´para el profe

log ""
log "--- sudo ---"
# -n evita que pida contraseña; si no tiene permiso, va directo al 'else'
if sudo -n -u "$USUARIO" sudo -l &>/dev/null; then
   fail "Tiene privilegios sudo (revisar!)"
else
   ok "Sin privilegios sudo (Cuenta restringida correctamente)"
fi
